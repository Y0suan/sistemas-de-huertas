...
function Categories({ swal }) {
  ...
  async function saveCategory(ev){
    ...
    if (editedCategory) {
      ...
      await axios.put('/api/referentes', data);
      setEditedCategory(null);
    } else {
      await axios.post('/api/referentes', data);
    }
    setName('');
    setParentCategory('');
    setProperties([]);
    ...
    swal.fire({
      ...
      reverseButtons: true,
        ...
        if (result.isConfirmed){
            const {_id} = Category;
            await axios.delete('/api/referentes?_id='+_id);  
            fetchCategories();
        }
    });
  }
  ...
  return (
    <Layout>
      ...
      <label>
        {editedCategory
          ? `Editar Un Referente ${editedCategory.name}`
          : 'Agrega Un Referente'
        }
      </label>
